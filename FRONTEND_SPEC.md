...
- ⛔ **백엔드 없음** — API 자체가 없어 화면/기능 보류 (M2 백엔드 의존)
...
| 에셋 등록 (Upload) | ✅ 폼+태그+카테고리 | mock 톤 유지 | `/posts`, `/files/upload` | ✅ |
...
- **M1 MVP 흐름은 3단계 직행**: `요청됨(REQUESTED) → 제작중(IN_PROGRESS) → 완료(COMPLETED)`
  - 요청됨→제작중: `PATCH /requests/{id}/assign` (TA 본인이 수락)
  - 제작중→완료: **완료 버튼 없음** — `POST /posts` 에 `linkedRequestId` 넣으면 자동 완료
- `IN_REVIEW(검토중)`: enum에만 존재, **v1.1 도입 검토** (M1 탭/흐름에서 제외)
- `REJECTED(반려)`: reject/reopen API **M1 미구현** (탭 제외, badge만 유지)
...
3. ✅ 에셋 게시판/상세/등록 + 3D 뷰어 (S3키 살아나면 검증)
...
> M2 페이지별 업그레이드 1~7 **완료**. 남은 건 백엔드 의존 기능(2번 표) 활성화뿐.
