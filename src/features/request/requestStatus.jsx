...
export const STATUS = {
  ...
  IN_REVIEW:   { label: '검토중', variant: 'yellow' },  // enum에만 존재 · 추후 도입 검토 (현재 미사용)
  IN_PROGRESS: { label: '제작중', variant: 'blue' },
  COMPLETED:   { label: '완료',   variant: 'green' },
  REJECTED:    { label: '반려',   variant: 'red' },      // reject/reopen API는 현재 미구현
}

// 현재 MVP 상태 흐름 (직행 3단계): 요청됨 → 제작중 → 완료
//   요청됨→제작중 = PATCH /assign,  제작중→완료 = post 작성 시 자동(완료 버튼 없음)
export const STATUS_FLOW = ['REQUESTED', 'IN_PROGRESS', 'COMPLETED']

// 현재 필터 탭: 전체 + 3단계 (IN_REVIEW/REJECTED는 탭 제외, '전체'에는 노출)
